...
def Required_Features(images, name):
    ...
    for i, face_ in enumerate(detected_faces):
        try:
            x, y, w, h = face_.left(), face_.top(), face_.width(), face_.height()
            left = x
            right = x + w
            bottom = y
            top = y + h
            image = images[left: right, bottom: top]
            # cv2.imshow("", image)
            # cv2.waitKey()
            cv2.imwrite("Aligned_Faces/{}.jpg".format(name), cv2.resize(image, (140, 140)))
            return image
        except:
            return None
...
Pkl_Filename = "Face_detector_1.pkl"
